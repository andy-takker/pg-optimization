...
Компромисс: `hsf=on` спасает отчётные запросы на реплике ценой удержания «мусора» на мастере (риск раздувания) — отсюда три типовых профиля (горячий резерв / OLTP-балансировка / реплика для отчётов) с разными `hot_standby_feedback` и `max_standby_streaming_delay`.
